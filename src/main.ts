...
// --- Main Menu Logic ---
const displayableZones: (Zone & { available: boolean, description: string, image?: string })[] = [
  {
    ...ZONES["meadow-1"],
    available: true,
    description: "A once peaceful meadow, now teeming with shadowy creatures.",
    image: "assets/zones/meadow.png" // Placeholder path
  },
  {
    id: "forest-of-whispers",
    name: "Soon...",
    movesToBoss: 0,
    boss: ZONES["meadow-1"].boss, // Placeholder
    baseMonsterHp: 0,
    baseMonsterAtk: 0,
    available: false,
...
function updateZoneDisplay() {
  ...
  const zoneImageEl = document.getElementById("zone-image-placeholder") as HTMLElement;
  const startButton = document.getElementById("start-zone-button") as HTMLButtonElement;
  ...
  if (nextButton) nextButton.disabled = currentZoneIndex === displayableZones.length - 1;

  // Update win counter specifically for meadow
  const winCounter = document.querySelector("#zone-name + #win-counter-meadow") as HTMLElement | null;
  if(winCounter) {
...
async function init() {
  ...
  setupMenuControls();
  updateZoneDisplay();
  updateMenuUI(); // This function is now only for meta-related UI updates if any
}
